...
const Home = () => {
  ...
  const fetchManhwaData = async (page) => {
    try {
      ...
      const response = await axiosInstance.get(`/manga?limit=${itemsPerPage}&offset=${offset}`); // Use the custom axios instance
      ...
      const manhwaDataWithDetails = await Promise.all(
        response.data.data.map(async (manhwa) => {
          ...
          if (coverRelationship) {
            try {
              const coverResponse = await axiosInstance.get(`/cover/${coverRelationship.id}`);
